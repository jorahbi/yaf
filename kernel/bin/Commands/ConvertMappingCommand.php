...
use Commands\Proxy\DoctrineCommand;
 ...
 */
class ConvertMappingCommand extends DoctrineCommand
{
    /**
     * {@inheritDoc}
     */
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        /*$destPath = APPLICATION_PATH;
        $type = $input->getArgument('mapping-type') ? $input->getArgument('mapping-type') : 'yml';
        if ('annotation' === $type) {
            $destPath .= '/application/entity';
        } else {
            $destPath .= '/conf/resources/orm';
        }
        if ('yaml' === $type) {
            $type = 'yml';
        }*/
        ...
        if ($metadata) {
            ...
            foreach ($metadata as $class) {
                ...
                if ('annotation' === $type) {
                    ...
                    $path = $destPath.'/' . 'Entity.' . str_replace('\\', '.', $className).'.dcm.'.$type;
                }
